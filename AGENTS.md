...
8. **Every controller action must have a complete `#[OA\...]` PHP Attribute annotation** — the CI `swagger.yml` will fail without it. Use `#[OA\Get]`, `#[OA\Post]`, etc. (NOT docblock `@OA\` syntax). See `.agents/workflows/openapi-annotations.md` for templates and rules.
